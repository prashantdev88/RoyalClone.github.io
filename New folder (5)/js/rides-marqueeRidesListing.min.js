$(function(){
    function marqueeRIdeListingAnalytics(ctaTextAn,nextPageURLAn,filterTextAn){
        dataLayer.push({
            'event':'marqueeRidesListing',
            'eventCategory':'marqueeRidesListing',
            'eventAction':ctaTextAn,
            'eventLabel':nextPageURLAn,
            'filter':filterTextAn   
        });
    }
    $(document).on('click','.listingCardAnalytics .list-item',function () {
        let ctaText = $(this).children("a").children(".card-comp-txt").children("h2").text().trim();
        let nextPageURL = $(this).children("a").attr("href");
        if(nextPageURL == undefined){
            nextPageURL =""
        }
        let filterText = $(this).parent(".listingCardAnalytics").siblings(".marquee-filter").children(".marqueeFilterAna").children(".active").attr("data-filter");
        marqueeRIdeListingAnalytics(ctaText,nextPageURL,filterText);
    });

    $(document).on('click','.marqueeFilterAna .filter-item',function () {
...
$(document).ready(function () {
    ...
    $(".list-item").each(function () {
        var date = $(this).attr("data-start-date");
        var interested = $(this).attr("data-popular-rank");
        var html = $(this).html();
        var listHTML = '<div class="col-xl-2 col-md-4 col-sm-12 list-item" data-start-date="' + date + '" data-popular-rank="' + interested + '"> ' + html + '</div>';
        var obj = { date: date, item: listHTML };
        allList.push(obj);
    });

    var dateUrl = $(".list-item").attr("get-date-api");
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            $(".list-item").each(function () {
                ...
                var listHTML = '<div class="col-xl-2 col-md-4 col-sm-12 list-item" data-start-date="' + date + '" data-popular-rank="' + interested + '"> ' + html + '</div>';
                var obj = { date: date, item: listHTML };
                var parsedDate =  Date.parse(date.replace("'"," "));
                var htmlDate = moment(parsedDate);
                if (htmlDate > todayDate) {
    ...
    $(".filter-item").click(function () {
        ...
        if ($(this).attr("data-filter") == "all") {
            ...
            for (var i = 0; i < allList.length; i++) {
                $(".row.no-gutters").append(allList[i].item);
            }
        } else if ($(this).attr("data-filter") == "upcoming") {
            $(".row.no-gutters").html("");
            for (var i = 0; i < upcomingList.length; i++) {
                $(".row.no-gutters").append(upcomingList[i].item);
            }
        }
        else if ($(this).attr("data-filter") == "completed") {
            $(".row.no-gutters").html("");
            for (var j = 0; j < completedList.length; j++) {
                $(".row.no-gutters").append(completedList[j].item);
            }
        }
    });
    ...
    function desktopView() {
        if ($(window).width() > 1090) {
            $(".marquee-ride-list .card-comp").each(function () {
                $(document).on('mouseenter','.inner-image',function (event) {
                    event.preventDefault();

                    if ($('.marquee-ride-list .card-comp').hasClass('mr-open')) {
                        $('.marquee-ride-list .card-comp').removeClass('mr-open');
                    }
                    $(this).parent().closest('.card-comp').addClass('mr-open');

                });

                $(document).on('mouseenter','.marquee-ride-slide',function () {
                    $(this).parent().closest('.card-comp').addClass('mr-open');
                });

                $(document).on('mouseleave','.inner-image',function () {
                    if ($('.marquee-ride-list .card-comp').hasClass('mr-open')) {
                        $('.marquee-ride-list .card-comp').removeClass('mr-open');
                    }
                });

                $(document).on('mouseleave','.marquee-ride-slide',function () {
                    $(this).parent().closest('.card-comp').removeClass('mr-open');
                });
            });
        }
        else {
    ...
    $(window).on("load resize scroll", function (e) {
        ...
        $(".marquee-ride-list .match-height").each(function () {
            if ($(this).height() > maxHeight) {
                maxHeight = $(this).height();
            }
        });

        $(".marquee-ride-list .match-height").height(maxHeight);
    });
    //end of match height

    //Marquee list justify-content-center
    if ($('.marquee-ride-list .col-xl-2').length > 4) {
        $('.marquee-ride-list .row.no-gutters').removeClass('justify-content-center');
    }
    else {
        $('.marquee-ride-list .row.no-gutters').addClass('justify-content-center');
    }
